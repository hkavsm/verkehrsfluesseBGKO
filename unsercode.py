...
# DEBUG:
# print(requests.get('https://api.mapbox.com/search/geocode/v6/forward?q=Marktplatz&access_token=pk.eyJ1IjoiaGthdnNtIiwiYSI6ImNtOWpvM2xzdjA0djQydXNiaTJ4bWtnOXYifQ.cBgoMPOQ70fiYOZrBKzDLA&bbox=8.4002,49.008,8.4068,49.0104').json())
# kein Ergebnis für Marktplatz in Karlsruher Innenstadt (nur der in Durlach für größere BBOX)
...
#    {'Locname': 'Staatliche Kunsthalle Karlsruhe', 'Lat': None, 'Lon': None},    # falsch: "Karlsruhe" (49.009716, 8.401216)
    ...
    {'Locname': 'Alter Schlachthof', 'Lat': findlatitude('Alter%20Schlachthof'), 'Lon': findlongitude('Alter%20Schlachthof')},    # schon richtig, aber Mapbox sucht anscheinend nur nach Straßen und Google-API ist zu restriktiv & teuer
...
# Als GeoDataFrame zum Shapefile-Abgleich der Koordinaten
unserdf = pd.DataFrame(locations)
unsergdf = gpd.GeoDataFrame(unserdf, geometry=[Point(laenge, breite) for laenge, breite in zip(unserdf['Lon'], unserdf['Lat'])], crs='EPSG:4326').to_crs(epsg=25832)
kashapefilegdf = gpd.read_file("Stadtteile_SHP/Stadtteile_Karlsruhe.shp")    # Einlesen der Shapefile-Datei der Stadt Karlsruhe mit den Stadtteilen
...
def finddistrict(place):
    for _, row in joinedgdf.iterrows():    # iterrows() von Pandas (nicht Py), wird von GeoPandas benutzt; 2D-Array, da sonst durch Spalten statt Zeilen iteriert wird
...
for person_id in range(1, num_people + 1):
    ...
    trip_locations = random.sample(locations, k=num_trips + 1)    # locations[0]? wegen wörterbuch statt liste
    ...
    for trip_index in range(num_trips):
        ...
        start_district = finddistrict(start_location)
        end_district = finddistrict(end_location)
        ...
        person_trips.append({
            ...
            'Startviertel': start_district,
            'Zielviertel': end_district
        })
